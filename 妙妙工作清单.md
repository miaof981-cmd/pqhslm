...
- 📄 **问题定位报告v2-云端迁移全局扫描.md**：42个文件880+处问题
  - **P0严重**：env=dev/useMockData=true、本地缓存读515+/写200+处
  - **P1高优**：28页面未迁移cloudAPI、utils/api.js降级逻辑
  - **P2中优**：字段不一致124+处、异步await缺失5处
- 📄 **日志插入diff.md**：✅全局日志系统已就绪（app.js/cloud-api.js/orderManager已插入）
- 📄 **全局扫描执行总结.md**：完整任务执行报告

- 📄 **后端上线前-审查清单.md**：5个致命问题+2个高危问题
  - **P0致命（禁止上线）**：
    1. env开关仍为mock模式
    2. 索引字段名错误（orderId→id、openid→_openid）
    3. 订单双写本地+云端
    4. 客服数据只存本地
    5. userId自增无事务保护
  - **P1高危（可暂缓）**：
    1. 客服分配无负载均衡
    2. 会员/商品到期无自动处理
  - **已完成**：8个云函数+前端API迁移+70+方法封装
  - **综合评分**：5/8模块可上线，3模块致命问题
  - **修复时间**：P0需2-3天，P1需1-2天

- 📄 **云数据库权限-最终修正版.md**：多角色权限精准核对（用户审查）
  - **第一次修正问题**：没有区分公开/私有数据
  - **第二次修正问题**：忽略了多角色访问需求（画师、客服、管理员）
  - **最终方案**：
    - 5个公开展示集合 → "所有用户可读，仅创建者可写"
    - 8个复杂权限集合 → "仅创建者可读写" + 云函数API控制
    - 新增 system_admin 管理员表（共13个集合）
  - **关键修正**：
    - users：管理员需查看所有用户
    - orders：买家/画师/客服/管理员按角色可见
    - income_ledger/withdraw_records：管理员需审核
    - reward_records：打赏人、画师、管理员可见
  - **提供资源**：
    - 云函数权限控制示例代码（5个函数）
    - 自定义JSON规则（后期优化用）
    - 详细权限对比表
