...
- ✅ **修复首页分类显示+搜索功能增强**：
  ...
  - **问题2**：画师名下有多个商品时，部分商品（如"蓝色"）搜不到
    - 原因1：搜索页面未过滤下架商品（与首页不一致）
    - 原因2：搜索维度不够（只有名称、分类、画师、标签）
    - 修复1：只加载`isOnSale !== false`的商品
    - 修复2：新增搜索维度：商品ID + 规格名称（如"蓝色"、"大号"）
  - **影响文件**：
    - miniprogram/utils/category-service.js
    - miniprogram/pages/search/index.js
  - **预期效果**：
    - 首页商品分类正常显示
    - 可以通过规格名称搜索商品（如搜"蓝色"能找到蓝色规格的商品）
    - 可以通过商品ID搜索（如搜"product_xxx"）
  - **已提交**：commit 4c61f91，已推送到main分支
