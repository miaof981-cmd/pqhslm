...
- ✅ **画师名字显示错误治本修复（三层防御）**：
  - **根本原因**：微信 `getUserProfile` API 返回的 `nickName` 有时是英文占位符（emoticon、cat_xxx、portrait）
  - **问题链路**：微信授权 → 保存 userInfo → 商品编辑读取 → 写入 artistName → 商品列表显示错误
  - **治本方案**：三层防御机制
    - **第1层：源头验证**（app.js `getWxUserInfo`）：
      - 新增 `isInvalidNickName` 函数检测非法昵称（emoticon、portrait、cat_xxx、纯英文+数字）
      - 微信授权后立即验证 `nickName`，如非法则从 users/applications 恢复
      - 无法恢复时使用 `用户{userId}` 作为默认值
    - **第2层：保存兜底**（product-edit `getCorrectArtistName`）：
      - 商品保存时多源读取画师名字（applications → users → userInfo → 默认值）
      - 每个来源都经过 `isInvalidNickName` 验证
      - 保证新上传的商品 artistName 一定合法
    - **第3层：定期清洗**（app.js `cleanInvalidArtistNames`）：
      - 启动时自动扫描所有商品的 artistName
      - 检测到非法名字自动从 users/applications 重新读取
      - 静默修复，无需用户干预
  - **影响文件**：
    - app.js（新增 `isInvalidNickName`、修改 `getWxUserInfo`、新增 `cleanInvalidArtistNames`）
    - pages/product-edit/index.js（新增 `getCorrectArtistName`、修改商品保存逻辑）
  - **修复效果**：
    - ✅ 新用户授权后，nickName 一定是合法中文昵称
    - ✅ 新商品上传，artistName 一定正确
    - ✅ 已有脏数据启动时自动清洗
    - ✅ 单点修复变为系统性防御，根除问题根源
  - **验证步骤**：
    1. 打开控制台查看启动日志 `🧹 [启动自检] 已修复 X 个商品`
    2. 刷新商品列表，所有画师名字应显示中文昵称
    3. 新上传商品，画师名字自动使用正确的中文昵称
