...
- [ ] 删除 income_ledger 相关错误索引（如有）
- [ ] 删除 withdraw_records 相关错误索引（如有）
- [ ] 删除 users 相关错误索引（如有）
- [ ] 创建 orders.id_1 索引（唯一）
- [ ] 创建 income_ledger.id 索引（如需要）
- [ ] 创建 withdraw_records.id 索引（如需要）
- [ ] 创建 users._openid 索引（如需要）
- [ ] 创建 sequences 集合
- [ ] 添加 userId 序列记录（`{_id: 'userId', value: 1001}`）
- [ ] 验证 orders 集合查询性能
- [ ] 验证 userId 自增功能
